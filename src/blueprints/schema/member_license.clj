...
(defn ^{:added "2.4.8"} license-transitions [part]
  ;;TODO - make a move-out attribute on a license
  (concat
   (s/generate-schema
    [(s/schema
      member-license
      (s/fields
       [transition :ref :index
        "A ref to a license transition. A ref value indicates that the user is transitioning from this license to either a new one (room reassignment), or to no license at all (moving out)"]))
     ...
     (s/schema
      ...
      (s/fields
       ...
       [transition-date :instant :indexed
        "The date at which the current unit assigned to the member will become unoccupied."]))])
   ...
   [
    ...
    ;;transition types
    {:db/id (d/tempid part)
     :db/ident :member-license-transition.type/move-out}
    {:db/id (d/tempid part)
     :db/ident :member-license-transition.type/intra-xfer}
    {:db/id (d/tempid part)
     :db/ident :member-license-transition.type/inter-xfer}
    {:db/id (d/tempid part)
     :db/ident :member-license-transition.type/renewal}]))
